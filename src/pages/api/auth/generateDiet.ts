  ...
  "mealPlanOptions": [
    {
      ...
      "toNote": [/* unique top tips */ ],
      "topTips": [/* unique top tips */ ],
      "freeSugars": [/* unique top sugar free guidelines */ ],
      "fibre": [/* unique top fibre guidelines */ ]
    },
    {
      ... // Second meal plan option
    },
  ...
  try {
    ...
    if (trimmedContent.startsWith("{") && trimmedContent.endsWith("}")) {
      try {
        ...
        res.status(500).json({ error: "Failed to parse JSON from ChatGPT response" });
      }
    } else {
      res.status(500).json({
        error: "ChatGPT response is not in valid JSON format",
        raw: messageContent,
      });
    }
  } catch (error) {
